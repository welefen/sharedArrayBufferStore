...
module.exports = class SharedArrayBufferStore {
  ...
  getKeyStorePos() {
    ...
    const pos = parseInt(this.crypto.decode(prefix), '10') || 0;
  ...
  updateKeyStorePos(pos) {
    ...
    const buf = this.crypto.encode(pos);
  ...
  getKeyBuf(key) {
    ...
    return this.crypto.encode(`${keyPrefix}${key}`);
  }
  getRealKey(keyArray) {
    const str = this.crypto.decode(keyArray);
  ...
  getKeyStoreProps(key) {
    ...
    start = parseInt(this.crypto.decode(start), '10');
    length = parseInt(this.crypto.decode(length), '10');
    return [start, length, pos];
  }

  addKey(key, start, length) {
    key = this.getKeyBuf(key);
    start = this.crypto.encode(this.padStart(start, this.valuePrefixLength));
    length = this.crypto.encode(this.padStart(length, this.valuePrefixLength));
  ...
  updateValueBuffer(buffer, pos) {
    buffer = this.crypto.encode(buffer);
  ...
  get(key, encoding) {
    ...
    if (props) {
      ...
      data = this.crypto.decode(result, encoding);
    }
    this.unlock();
    return data;
  }
  ...
  set(key, value, encoding) {
    this.lock();
    key = this.getKeyBuf(key);
    value = this.crypto.encode(value, encoding);
